...
Found in the final whole-branch review and fixed in the same wave. The full findings list is in the plan folder's `final-fix-brief.md`.

- **The rail's height lookups are batched.** `Rail.advance` used to call `ground(x, z)` three times a frame (once redundantly), each a 20-octave noise evaluation on a single point (~1.5 ms). `Rail` now takes an optional `ground_many(xs, zs)` and looks up the focus point and the trailing eye's floor point together, once per `advance` (and once at construction), caching both heights so `focus`/`eye`/`_eye_floor` never recompute them between advances. Measured with the plan's `bench_frames.py` at 400 frames from `--start 1200`: rail cost median 4.47 ms -> 1.52 ms, max 12.16 ms -> 1.97 ms; frames over 16.7 ms in that stretch dropped from 13 to 0.
- **New GL coverage.** `test_gfx.py` only ever viewed a patch 12 m from a ri centre, so cho and ri borders (drawn only where a coarser tier is loaded further out) were never exercised. It now has a second loader focus near a ri corner whose shaku window holds both a cho-only border and a genuine ri border (its negative q also exercises `split_origin`'s floor on negatives), a border-colour check parametrized over all four levels, an orthographic view straddling the world's rim (ri radius 12) checking that nothing draws past it, and a pure-Python test that every window's boundary-cell vertices lie at least `MORPH_END` parent widths from the loader focus, for a few foci including the rail start.
- **The rail's default start also runs along a cho edge.** Not just the shaku window (already noted above): the loader's cho (the cell `up(focus, SHAKU, CHO)`) also toggles between two candidates for the first few minutes, starting around t = 25 s, because the same north-heading alignment sits almost exactly on a cho/cho tie as well as a ken/ken one. Each toggle changes the ken tier's window (its cho parents) by 7 chunks each way, and a ken chunk costs about 10 ms to generate. This is correct under the no-hysteresis rule (see the open question below), not a bug.
- **Limitation for exp-05:** rendering is centred on the camera. The geomorph distance and the ±4-cell loaded-children lookup textures are both relative to the single camera loader; a second loader's windows would load correctly (loading has no such assumption) but would not draw correctly, because the renderer has no way to draw geomorphing or tier handovers relative to a second origin. Fine for exp-03 (one loader, the camera), but exp-05 will need to address this before adding a second loader.
